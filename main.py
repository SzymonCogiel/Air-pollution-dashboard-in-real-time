...
if __name__ == '__main__':
    ...
    dic = r.json()
    pomiarowe_PM10 = pd.json_normalize(dic['values'])
    pomiarowe_CO = pomiarowe_PM10.copy()
    ...
    for sen_id in stanowiska['id'].unique()[:400]:
        ...
        if dane_pomiarowe['key'] == 'PM10':
            try:
                val = pd.DataFrame.from_dict(pd.json_normalize(dane_pomiarowe['values'])['value'])
                name = stacje[stacje['id'] == stanowiska[stanowiska['id'] == sen_id].iloc[0]['stationId']]\
                    .iloc[0]['stationName']
                val.rename(columns={'value': str(name)}, inplace=True)
                pomiarowe_PM10 = pomiarowe_PM10.merge(val, left_index=True, right_index=True)
        ...
        if dane_pomiarowe['key'] == 'CO':
            try:
                ...
                name = stacje[stacje['id'] == stanowiska[stanowiska['id'] == sen_id].iloc[0]['stationId']]\
                    .iloc[0]['stationName']
                val.rename(columns={'value': str(name)}, inplace=True)
                pomiarowe_CO = pomiarowe_CO.merge(val, left_index=True, right_index=True)
            except Exception as e:
                logger.error(e)
        # print(i)
        # i += 1
    ...
    pomiarowe_PM10 = pomiarowe_PM10.drop(['value'], axis=1)
    pomiarowe_PM10.to_csv('pomiarowe_PM10.csv')
    pomiarowe_CO = pomiarowe_CO.drop(['value'], axis=1)
    ...
    item_to_update3 = 'bf82d5e07b9d4c97b0acb95ab79eee63'
    ...
    item_to_update4 = 'fae4d986da004b7b8747849edad75759'
